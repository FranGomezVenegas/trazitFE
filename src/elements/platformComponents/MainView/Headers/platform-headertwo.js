...
import {
  SOP_pendingSOPTab,
  SOP_userMySOPTab,
} from "../../../../elements/platformComponents/Tabs/tabs-settings";
...
class PlatformHeadertwo extends FieldsMethods(
  ...
  stateChanged(state) {
    ...
    if (!state.SOPS.userAllSop) {
      ...
      if (state.SOPS.userAllSop.length == 0) {
        this.contAllUserSOPs = 0;
      } else {
...
<!--                    <field-controller .field="${item}" @field-button-clicked="${
  ...
  getHeaderInfo(user, session) {
    ...
    if (session) {
      this.platformHeaderFields.fieldsCenter[1].label_es =
        "SESIÓN Id: " + session.sessionId + " Fecha: " + session.startDate;
  ...
  crearTab(e) {
    if (!isTabOpenableWhenNotSopCertified(e.detail.procedure)) {
      ...
      if (
        e.detail.procEvent.sops_passed == null ||
        e.detail.procEvent.sops_passed == false
      ) {
