...
export const FunctionsEnvMonitSamples = (superClass) =>
  class extends EnvMonitModuleDefinition(
    ...
    enterResultChecker(rawValue, selectedRow) {
      ...
      var messageErr = [];
      if (
        selectedRow.min_val_allowed != undefined &&
        String(selectedRow.min_val_allowed).length > 0
      ) {
        var minStrict = true;
        if (
          selectedRow.min_allowed_strict != undefined &&
          selectedRow.min_allowed_strict == false
        ) {
          minStrict = false;
        }
        if (!minStrict && rawValue < selectedRow.min_val_allowed) {
          messageErr.message_en = this.resultCheckerMessages.valueLessThanMinAllowed.message_en
            .replace("<*1*>", String(rawValue))
            .replace("<*2*>", String(selectedRow.min_val_allowed));
      ...
      if (
        selectedRow.max_val_allowed != undefined &&
        String(selectedRow.max_val_allowed).length > 0
      ) {
        var maxStrict = true;
        if (
          selectedRow.max_allowed_strict != undefined &&
          selectedRow.max_allowed_strict == false
        ) {
          maxStrict = false;
        }
        if (!maxStrict && rawValue > selectedRow.max_val_allowed) {
          messageErr.message_en = this.resultCheckerMessages.valueGreaterThanMaxAllowed.message_en
            .replace("<*1*>", String(rawValue))
            .replace("<*2*>", String(selectedRow.max_val_allowed));
    ...
    microorgActionOnSel(e) {
      if (e.detail.buttonName == "ADD_MICROORGANISM") {
        this.addMicroorganism(e);
      }
      if (e.detail.buttonName == "ADD_ADHOC_MICROORGANISM") {
    ...
    checkAdditionIsCorrect(object, number) {
      ...
      if (
        number == undefined ||
        object == undefined ||
        object.microorganism_count == undefined ||
        object.raw_value == undefined
      ) {
        var errMsg = {
          message_en:
            "Wrong data passed for the addition, review checkAdditionIsCorrect in functions-env-minit-samples",
          message_es:
            "Recibida información incorrecta para añadir microorganismos, revisar checkAdditionIsCorrect en functions-env-minit-samples",
        };
      ...
      if (sumVal <= Number(object.raw_value)) {
        return true;
      }
      var errMsg = {
        message_en:
          ...
          " what is not allowed.",
        message_es:
          "Está adición sumaría un total de " +
          sumVal +
    ...
    addMicroorganism(e) {
      ...
      var allowed = this.checkAdditionIsCorrect(
        this.selectedObject,
