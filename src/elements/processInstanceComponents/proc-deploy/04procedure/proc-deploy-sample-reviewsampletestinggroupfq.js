...
import '../03config/proc-deploy-sample-reviewsampletestinggroupfq-settings';
import {windowSettings} from '../03config/proc-deploy-sample-reviewsampletestinggroupfq-settings'; //'../03config/config-process.js';
...
class procDeploySampleReviewsampletestinggroupfq extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))) {
    ...
    loadSamplingTable(){
        ...
        this.getAllSamplesStageSampling(this.tableDefinition,this.samplesStagesReduxVariables.SAMPLES_PENDINGREVISION_TESTINGGROUPFQ);
    } 
    ready() {
        super.ready();
        this.loadSamplingTable();
...
customElements.define('proc-deploy-sample-reviewsampletestinggroupfq', procDeploySampleReviewsampletestinggroupfq);
