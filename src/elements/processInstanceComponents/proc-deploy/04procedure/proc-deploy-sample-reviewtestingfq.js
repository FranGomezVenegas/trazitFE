...
import '../03config/proc-deploy-sample-reviewtestingfq-settings';
import {windowSettings} from '../03config/proc-deploy-sample-reviewtestingfq-settings'; //'../03config/config-process.js';
...
class procDeploySampleReviewtestingfq extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))) {
    ...
    loadSamplingTable(){
        ...
        this.getAllSamplesStageSampling(this.tableDefinition,this.samplesStagesReduxVariables.SAMPLEANALYSIS_PENDINGREVISION_FQ);
    } 
    ready() {
        super.ready();
        this.loadSamplingTable();
