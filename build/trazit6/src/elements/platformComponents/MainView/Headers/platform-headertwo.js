define([
  ...
  "../../../../config/platform/main-layout/plaftorm-config.js",
  "../../Redux/actions/app_actions.js",
  "../../Redux/actions/notifications_actions.js",
  ...
  _tabsFunctions,
  _fieldsMethods,
  _notificationsPaneSettings,
  _platformConfig,
  _platformHeadertwoSettings,
  _sopIconAndBadge,
  _storeConsola,
  ...
  "use strict"; //import {addTab,setCurrentTab} from '../../Redux/actions/tabs_actions';
   ...
   * @demo
   *
   */ class PlatformHeadertwo extends (0, _fieldsMethods.FieldsMethods)(
    ...
    stateChanged(state) {
      ...
      if (null != state.app.user.appProcedureList) {
        this.procedureList = state.app.user.appProcedureList; //console.log('procedureList', this.procedureList);
      }
      ...
      if (0 == state.SOPS.userPendingSop.length) {
        this.contPendingUserSOPs = 0;
        this.showPendingSOP = !1; //this.iconName=this.sopPaneIconAndBadge_icons.iconGreen;
      } else {
     ...
     */ constructor() {
      ...
      this.platformHeaderFields =
        _platformHeadertwoSettings.platformHeaderFields;
      this.personalOption =
        _platformHeadertwoSettings.platformHeaderFields.personalOption;
      this.selectedLanguage = "";
      this.notificationPaneIsEmpty = !0;
      this.paneTitle = _notificationsPaneSettings.notificationsPaneTitle;
      this.procedureList = [];
      this.appLogOut_logOutMessage = _plaftormConfig.appLogOut_logOutMessage; //console.log('platformHeaderFields', this.platformHeaderFields);
    }
    static get styles() {
      return [
        // super.styles,
        _platformHeadertwoSettings.platformHeadertwoStyle,
        _litElement.css` 
            a.textRed {
              color:red;
              text-size:1.8vh;
    ...
    <div class="main">  
      <div class="left">
        ...
        <div class="logo">
        <!-- <img class="formFieldLogoCircle" src="./images/header/trazit-removebg.png"> -->
          ${this.platformHeaderFields.fieldsCenter.map(
            (item) => _litElement.html`<p class="sessioninfo"><b>
          ${this.labelValue(this.selectedLanguage, item)}
          <!-- <field-controller .field="${item}" @field-button-clicked="${
              this.doLogout
            }" 
          @avatar-item-clicked="${
            this.platformHeaderRightClicked
          }"></field-controller> -->
          </b></p>`
          )}
        </div>
      ...
      <div class="center">
        ...
        <ul>
        ${
          this.platformHeaderFields.proceduresOption.display
            ? _litElement.html`              
          <li class="circle"><a> ${this.tabLabelValue(
            this.selectedLanguage,
            this.platformHeaderFields.proceduresOption.tab
          )}</a>
          <ul>
          ${
            this.procedureList.procedures
              ? _litElement.html`
            ${this.procedureList.procedures.map(
              (item) => _litElement.html`
            ${
              0 < item.definition.length
                ? _litElement.html`
            <li><a>
              ${this.labelValue(
                this.selectedLanguage,
              ...
              <ul>
              ${
                item.icons_up != void 0
                  ? _litElement.html`
                ${item.icons_up.map(
                  (
                    iconup
                  ) => _litElement.html`<li><a .procedure="${item}" .procevent="${iconup}" @click="${
                    this.procedureTab
                  }"> <!--(item, iconup)-->
                <!-- <field-controller style="padding-top: 0px; padding-bottom: 0px;" id="${
                  iconup.name
                }" @field-button-clicked="${this.crearTab}" tab-index="${
                    item.index
                  }" .field="${iconup}" .procedure="${item}"></field-controller> -->                           
                <!-- ${this.labelValue(
                  this.selectedLanguage,
                  iconup
                )}                        -->
                <paper-icon-button style="${this.iconStyleDefinition}" icon="${
                    iconup.icon_name
                  }" title="${this.labelValue(this.selectedLanguage, iconup)}"
                disabled="${iconup.read_only}" value="${
                    iconup.name
                  }" ></paper-icon-button>               
                </a></li>`
                )}
              ...
              ${
                item.definition != void 0
                  ? _litElement.html`
                ${item.definition.map(
                  (
                    item2
                  ) => _litElement.html`<li><a .procedure="${item}" .procevent="${item2}" @click="${
                    this.procedureTab
                  }"> <!--(item, item2)-->
                  ${this.labelValue(this.selectedLanguage, item2)}  
  <!--                <field-controller title="${
    this.fieldTitlePendingSOP
  }" style="padding-top: 0px; padding-bottom: 0px;" id="${
                    item2.name
                  }" @field-tree-list-clicked="${this.crearTab}" tab-index="${
                    item.index
                  }" .field="${item2}" procedure="${item}"></field-controller>            -->
                </a></li>`
                )}
              `
                  : ``
              ...
              ${
                ...
                <paper-icon-button style="${this.iconStyleDefinition}" icon="${
                    icons_down.icon_name
                  }" title="${this.labelValue(
                    this.selectedLanguage,
                    icons_down
                  )}"
                disabled="${icons_down.read_only}" value="${
                    icons_down.name
                  }" ></paper-icon-button>               
                </a></li>`
                )}
        ...
        ${
          this.platformHeaderFields.notificationsOption.display
            ? _litElement.html`              
          <li class="circle"><a>${this.tabLabelValue(
            this.selectedLanguage,
            this.platformHeaderFields.notificationsOption.tab
          )} ${this.numNotifications}</a>
          <ul>
          ${
            this.notificationPaneIsEmpty
              ? _litElement.html`
            <li><a>${this.labelValue(
              this.selectedLanguage,
              this.paneTitle.empty
            )}</a></li>
          `
              : _litElement.html`
            ${this.notifications.map(
              (item) => _litElement.html`<li><a 
                class="${this.textColor(item[1])}"> ${this.notifText(
                item[1]
              )}                
            </a></li>`
            )}
        ...
        ${
          this.platformHeaderFields.sopOption.display
            ? _litElement.html`
          <li class="circle">
            <a @click="${this.AllUserSOPClicked}">${this.tabLabelValue(
                this.selectedLanguage,
                this.platformHeaderFields.sopOption.tab
              )}</a>
            ${
              0 < this.contPendingUserSOPs
                ? _litElement.html`   
              <a style="color:red;" title="${this.tabLabelValue(
                this.selectedLanguage,
                _tabsSettings.SOP_pendingSOPTab
              )}" @click="${this.pendingSOPClicked}" >${
                    this.contPendingUserSOPs
                  }</a>
            `
                : ``
            }            
            <a title="${this.tabLabelValue(
              this.selectedLanguage,
              _tabsSettings.SOP_userMySOPTab
            )}"  @click="${this.AllUserSOPClicked}" >${this.contAllUserSOPs}</a>
            <input type="checkbox" id="btn-1">
            <ul> 
              ${this.platformHeaderFields.sopOption.options.map(
                (item) => _litElement.html`<li><a>
        ...
        ${
          this.platformHeaderFields.personalOption.display
            ? _litElement.html`
          <li class="circle">
            <a href="#">${this.tabLabelValue(
              this.selectedLanguage,
              this.platformHeaderFields.personalOption.tab
            )}</a>
            <ul> 
              ${this.platformHeaderFields.personalOption.options.map(
                (item) => _litElement.html`<li><a .field="${item}" @click="${
                  this.platformHeaderRightClicked
                }">
              ${
                "avatar" == item.type
                  ? _litElement.html`
                <img class="formFieldAvatar"  src="${
                  item.source
                }" aligned="center"  height="30vh" width="30vw"> 
                ${this.labelValue(this.selectedLanguage, item)} 
              `
                  : `
                ${this.labelValue(this.selectedLanguage, item)} 
<!--                    <field-controller .field="${item}" @field-button-clicked="${
    ...
    pendingSOPClicked() {
      _store.store.dispatch(
        (0, _tabs_actions.addSystemTab)(_tabsSettings.SOP_pendingSOPTab)
      );
      _store.store.dispatch(
        (0, _tabs_actions.setCurrentTab)(_tabsSettings.SOP_pendingSOPTab)
      );
    }
    AllUserSOPClicked() {
      _store.store.dispatch(
        (0, _tabs_actions.addSystemTab)(_tabsSettings.SOP_userMySOPTab)
      );
      _store.store.dispatch(
        (0, _tabs_actions.setCurrentTab)(_tabsSettings.SOP_userMySOPTab)
      );
    }
    notifText(notif) {
      //console.log('notifText', notif);
      return (
        notif.category + " : " + this.labelValue(this.selectedLanguage, notif)
    ...
    procedureTab(e) {
      var procedure = e.currentTarget.procedure,
        procEvent = e.currentTarget.procevent;
      if (!(0, _platformConfig.isTabOpenableWhenNotSopCertified)(procedure)) {
        console.log(
          "isTabOpenableWhenNotSopCertified",
          (0, _platformConfig.isTabOpenableWhenNotSopCertified)()
        );
        if (null == procEvent.sops_passed || !1 == procEvent.sops_passed) {
          this.toastSuccessMessage(this.tabNotOpenableByCertification);
          return;
        }
      } // attributes[1]
      var esignRequired = !1;
      if (!procEvent.esign_required) {
        esignRequired = !1;
      } else {
        esignRequired = procEvent.esign_required;
      }
      var confirmUserRequired = !1;
      if (!procEvent.confirm_required) {
        confirmUserRequired = !1;
      } else {
        confirmUserRequired = procEvent.confirm_required;
      } //e.stopPropagation();
      _store.store.dispatch(
        (0, _tabs_actions.addTab)({
    ...
    doLogout() {
      ...
      _store.store.dispatch((0, _app_actions.doLogout)(this.selectedLanguage));
      _store.store.dispatch((0, _tabs_actions.doLogoutTab)());
      _store.store.dispatch((0, _notifications_actions.doLogoutNotification)());
    }
    getHeaderInfo(user, session) {
      if (
        user.userInfo &&
    ...
    platformHeaderRightClicked(e) {
      ...
      if (menuItem.tab) {
        _store.store.dispatch((0, _tabs_actions.addSystemTab)(menuItem.tab));
        _store.store.dispatch((0, _tabs_actions.setCurrentTab)(menuItem.tab));
      } else {
        this.doLogout();
      }
      return;
