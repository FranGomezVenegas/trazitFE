...
}getConfigVariablesAndVariablesSet(){this.callBackRefreshWindow=this.refreshWindow.bind(this);var data=[];this.callEndpoint(data,"ALL_ACTIVE_VARIABLES_AND_VARIABLES_SET")}refreshSelectedStudyOnGetProjects(projects){//console.log('refreshSelectedStudyOnGetProjects '); 
var state=_store.store.getState();if(state.genomaInstancia1.selectedStudy.name!=void 0){var selStudyName=state.genomaInstancia1.selectedStudy.name,selProjectName=state.genomaInstancia1.selectedStudy.project,selProject=projects.project.find(function(tab){return tab.name.toUpperCase()==selProjectName.toUpperCase()});if(selProject==void 0){return}var selStudy=selProject.study.find(function(tab){return tab.name.toUpperCase()==selStudyName.toUpperCase()});_store.store.dispatch((0,_genomaInstancia1_actions.setSelectedStudy)(selStudy));console.log("setSelectedStudy fired by getProjects")}}callEndpoint(data,endpointName){var endpoints_returningError=this.endpoints_returningError(),moduleEndpoints=[];//console.log('callEndpoint', 'data', data, 'endpointName', endpointName);
...
switch(endpointName){case"ALL_ACTIVE_VARIABLES_AND_VARIABLES_SET":_store.store.dispatch((0,_genomaInstancia1_actions.activeVariablesAndVariablesSet)(response.data));return;case"ALL_ACTIVE_PROJECTS":_store.store.dispatch((0,_genomaInstancia1_actions.activeProjects)(response.data));this.refreshSelectedStudyOnGetProjects(response.data);//store.dispatch(activeProjects_genoma_instancia1(response.data, endpointName));
