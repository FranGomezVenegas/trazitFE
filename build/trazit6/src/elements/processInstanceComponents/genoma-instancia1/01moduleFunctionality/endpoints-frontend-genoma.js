define([
  ...
  _exports.FrontendEndpointsModuleGenoma = void 0; //import {backendUrl, frontEndGenomaStudyUrl} from '../../../../config/platform/logic/api-config';
  const FrontendEndpointsModuleGenoma = (superClass) =>
    class extends (0,
      ...
      refreshSelectedStudyOnGetProjects(projects) {
        ...
        if (state.genomaInstancia1.selectedStudy.name != void 0) {
          ...
          _store.store.dispatch(
            (0, _genomaInstancia1_actions.setSelectedStudy)(selStudy)
          );
            ...
            if (200 == response.status) {
              ...
              switch (endpointName) {
                case "ALL_ACTIVE_VARIABLES_AND_VARIABLES_SET":
                  _store.store.dispatch(
                    (0,
                    _genomaInstancia1_actions.activeVariablesAndVariablesSet)(
                      response.data
                    )
                  );
                  return;
                case "ALL_ACTIVE_PROJECTS":
                  _store.store.dispatch(
                    (0, _genomaInstancia1_actions.activeProjects)(response.data)
                  );
  ...
  _exports.FrontendEndpointsModuleGenoma = FrontendEndpointsModuleGenoma;
});
