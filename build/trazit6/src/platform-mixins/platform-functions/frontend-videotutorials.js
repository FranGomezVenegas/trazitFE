define([
  ...
  "../../elements/platformComponents/Redux/actions/videotutorials_actions.js",
  ...
  _exports.FrontendVideoTutorials = void 0;
  /**
   * @mixinFunction
   * @polymer
   */ const FrontendVideoTutorials = (superClass) =>
    class extends (0, _apiSettings.ApiSettings)(
      ...
      getActiveVideoTutorials(data) {
        ...
        var apiUrl =
          ...
          _apiConfig.frontEndVideoTutorialsUrl +
          "?" +
          "actionName=ALL_ACTIVE_VIDEO_TUTORIALS"; //console.log('getActiveVideoTutorials', apiUrl, data.finalToken);
        axios
          ...
          .then((response) => {
            if (200 == response.status) {
              ...
              _store.store.dispatch(
                (0, _videotutorials_actions.allActiveVideoTutorials)(
                  response.data
                )
              );
      ...
      getSelectedUserIncidentDetail(data) {
        ...
        if (!data.incidentId) {
          var noIncident = [];
          _store.store.dispatch(selectedUserIncidentDetail(noIncident));
          return;
        }
        if (!finalToken) {
          return;
        ...
        axios
          ...
          .then((response) => {
            if (200 == response.status) {
              //console.log(response.data);
              _store.store.dispatch(selectedUserIncidentDetail(response.data));
              if (data && data.callBackFunction) {
                data.callBackFunction();
              }
              return;
  ...
  _exports.FrontendVideoTutorials = FrontendVideoTutorials;
});
