define([
  ...
  "../../elements/platformComponents/Redux/actions/incidents_actions.js",
  "../store/token-mixin.js",
  "../functions/toast-methods.js",
  "../apis/api-settings.js",
], function (
  _exports,
  _apiConfig,
  _store,
  ...
  _exports.FrontendIncidents = void 0;
  /**
   * @mixinFunction
   * @polymer
   */ const FrontendIncidents = (superClass) =>
    class extends (0, _apiSettings.ApiSettings)(
      ...
      fieldButtonClickedForIncidents(e) {
        ...
        switch (actionName) {
          ...
          case "NEW_INCIDENT": //console.log('NEW_INCIDENT');
            ...
            paramsUrl =
              paramsUrl + "&incidentTitle=" + this.formFields[0].value;
            paramsUrl =
              paramsUrl + "&incidentDetail=" + this.formFields[1].value;
            var storeCurrentState = JSON.stringify(_store.store.getState());
      ...
      frontEndIncidentsAPI(data) {
        var apiUrl =
          ...
          _apiConfig.frontEndIncidentsUrl +
          "?" +
          data.paramsUrl;
        if (!data.finalToken) {
          return;
        ...
        axios
          ...
          .then((response) => {
            if (200 == response.status) {
              ...
              _store.store.dispatch(
                (0, _incidents_actions.userOpenIncidents)(response.data)
              );
      ...
      getSelectedUserIncidentDetail(data) {
        ...
        if (!data.incidentId) {
          var noIncident = [];
          _store.store.dispatch(
            (0, _incidents_actions.selectedUserIncidentDetail)(noIncident)
          );
          return;
        }
        if (!finalToken) {
        ...
        axios
          ...
          .then((response) => {
            if (200 == response.status) {
              //console.log(response.data);
              _store.store.dispatch(
                (0, _incidents_actions.selectedUserIncidentDetail)(
                  response.data
                )
              );
              if (data && data.callBackFunction) {
  ...
  _exports.FrontendIncidents = FrontendIncidents;
});
