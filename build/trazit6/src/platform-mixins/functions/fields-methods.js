define(["exports"], function (_exports) {
  ...
  _exports.FieldsMethods = void 0;
  /**
   * @mixinFunction
   * @polymer
   */ const FieldsMethods = (superClass) =>
    class extends superClass {
      ...
      fieldValue(langApp, field, fieldName) {
        ...
        if ("en" == langApp) {
          if (field.label_en) {
            return field[fieldName + "_en"];
          }
          if (field.message_en) {
            return field.message_en;
          }
          console.log(
            "methods.js",
            "labelValue with no label_en recognized",
            "field=",
            field,
            "langApp=",
            langApp
        ...
        if ("es" == langApp) {
          if (field.label_es) {
            return field[fieldName + "_es"];
          }
          if (field.message_es) {
            return field.message_es;
          }
          console.log(
            "methods.js",
            "labelValue with no label_es recognized",
            "field=",
            field,
            "langApp=",
            langApp
      ...
      labelListValue(langApp, item) {
        ...
        if ("en" == langApp) return item.keyValue_en;
        if ("es" == langApp) return item.keyValue_es;
        return this.field.keyValue_en;
      }
      confidentialMaskValue(langApp) {
        var formValue_confidentialMaskDefault = "*******",
          formValue_confidentialMask_en = "*CONFIDENTIAL*",
          formValue_confidentialMask_es = "*CONFIDENCIAL*"; //        console.log('confidentialMaskValue', langApp);
      ...
      enable() {
        ...
        this.shadowRoot
          .getElementById(this.field.name)
          .removeAttribute("disabled"); //this.shadowRoot.getElementById(this.field.name).read_only=false;
      ...
      disable() {
        ...
        this.shadowRoot.getElementById(this.field.name).disabled = !0;
      }
    };
  _exports.FieldsMethods = FieldsMethods;
});
