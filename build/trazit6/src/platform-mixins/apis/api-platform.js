define([
  ...
  "../platform-functions/notification-obj.js",
  ...
  _exports.ApiPlatform = void 0;
  const ApiPlatform = (superClass) =>
        ...
        switch (actionName) {
          case endpoints_for_apiPlatform.userChangePassword.action_name: //'USER_CHANGE_PSWD':
            if (
              ...
              0 == selectedRow.newPassword.length
            ) {
          ...
          case endpoints_for_apiPlatform.userChangeEsign.action_name: //'USER_CHANGE_ESIGN':
            if (!selectedRow.newEsign || 0 == selectedRow.newEsign.length) {
              this.toastErrorMessage(
                endpoints_for_apiPlatform.userChangeEsign
                  .mandatory_field_message
              );
            }
      ...
      appBackEndCallAPI(data) {
        var endpoints_returningError = this.endpoints_returningError(),
          apiUrl =
            ...
            _apiConfig.appAuthenticateApiUrl +
            "?" +
            data.paramsUrl; //console.log('process-us>api-sample>sampleBackEndCallAPI', data.schemaPrefix, data.actionName, apiUrl, data.paramsUrl);
        axios
          .get(apiUrl)
          .then((response) => {
            ...
            if (200 == response.status) {
              if (
                ...
                _store.store.dispatch(
                  (0, _app_actions.updateFinalToken)(this.finalToken)
                );
              }
  ...
  _exports.ApiPlatform = ApiPlatform;
});
