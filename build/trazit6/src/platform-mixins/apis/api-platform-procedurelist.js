define([
  ...
  _exports.ProcedureList = void 0;
  /**
   * @mixinFunction
   * @polymer
   */ const ProcedureList = (superClass) =>
    class extends (0, _apiSettings.ApiSettings)(
      ...
      getProcedureList(data) {
        ...
        var apiUrl = _apiConfig.backendUrl + _apiConfig.appProcedureListApiUrl;
        if (!finalToken) {
          return;
        }
        axios
          ...
          .then((response) => {
            if (200 == response.status) {
              _store.store.dispatch(
                (0, _app_actions.setAppProcedureList)(response.data)
              );
  ...
  _exports.ProcedureList = ProcedureList;
});
