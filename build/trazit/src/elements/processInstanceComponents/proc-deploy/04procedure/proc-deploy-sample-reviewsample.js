...
import"../../../internalComponents/Grids/vaadingrid-lit-singleselect.js";import{FieldsMethods}from"../../../../platform-mixins/functions/fields-methods.js";import"../03config/proc-deploy-sample-reviewsample-settings.js";import{windowSettings}from"../03config/proc-deploy-sample-reviewsample-settings.js";//'../03config/config-process.js';
...
class procDeploySampleReviewsample extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))){static get properties(){return{tableDefinition:{type:Object,value:windowSettings},samplePendingSampleRevision:{type:Array,notify:!0},selectedObject:Object,callBackRefreshWindow:Object,selectedLanguage:{type:String},samplesStagesReduxVariables:{type:String,value:samplesStagesReduxVariables}}}stateChanged(state){this.selectedLanguage=state.app.user.appLanguage;if(null!=state.procDeploy){this.samplePendingSampleRevision=state.procDeploy.samplePendingSampleRevision}}static get template(){return html`            
            ...
            <div class="main">
                ...
                <vaadingrid-lit-singleselect class="grid" id="ProcDeploy-samplesampling" headerfields="{{tableDefinition.sampleFieldToDisplay}}" 
                    rowcontainer="{{samplePendingSampleRevision}}" selected-object="{{selectedObject}}">
                </vaadingrid-lit-singleselect>
            </div>  
        `}refreshWindow(){this.loadSamplingTable()}loadSamplingTable(){this.callBackRefreshWindow=this.refreshWindow.bind(this);this.getAllSamplesStageSampling(this.tableDefinition,this.samplesStagesReduxVariables.SAMPLES_PENDINGSAMPLEREVISION)}ready(){super.ready();this.loadSamplingTable()}}customElements.define("proc-deploy-sample-reviewsample",procDeploySampleReviewsample);// import {LitElement, html} from 'lit-element';
