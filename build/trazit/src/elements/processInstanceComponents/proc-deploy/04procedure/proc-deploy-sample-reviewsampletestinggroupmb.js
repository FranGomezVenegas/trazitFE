...
import "../03config/proc-deploy-sample-reviewsampletestinggroupmb-settings.js";
import { windowSettings } from "../03config/proc-deploy-sample-reviewsampletestinggroupmb-settings.js"; //'../03config/config-process.js';
import {
...
class procDeploySampleReviewsampletestinggroupmb extends FieldsMethods(
  ...
  static get properties() {
    return {
      ...
      samplePendingRevisionTheTestingGroupMB: { type: Array, notify: !0 },
  ...
  stateChanged(state) {
    ...
    if (null != state.procDeploy) {
      this.samplePendingRevisionTheTestingGroupMB =
        state.procDeploy.samplePendingRevisionTheTestingGroupMB;
    }
  }
  static get template() {
    return html`
      <style
        include="proc-deploy-sample-reviewsampletestinggroupmb-style"
      ></style>
      ...
      <div class="main">
        ...
        <vaadingrid-lit-singleselect
          ...
          rowcontainer="{{samplePendingRevisionTheTestingGroupMB}}"
  ...
  loadSamplingTable() {
    ...
    this.getAllSamplesStageSampling(
      ...
      this.samplesStagesReduxVariables.SAMPLES_PENDINGREVISION_TESTINGGROUPMB
    );
  }
  ready() {
    super.ready();
...
customElements.define(
  "proc-deploy-sample-reviewsampletestinggroupmb",
  procDeploySampleReviewsampletestinggroupmb
