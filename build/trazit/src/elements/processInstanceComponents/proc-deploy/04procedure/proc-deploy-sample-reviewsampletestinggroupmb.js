...
import"../../../internalComponents/Grids/vaadingrid-lit-singleselect.js";import{FieldsMethods}from"../../../../platform-mixins/functions/fields-methods.js";import"../03config/proc-deploy-sample-reviewsampletestinggroupmb-settings.js";import{windowSettings}from"../03config/proc-deploy-sample-reviewsampletestinggroupmb-settings.js";//'../03config/config-process.js';
...
class procDeploySampleReviewsampletestinggroupmb extends FieldsMethods(FunctionsEnvMonitSamples(FrontendEndpointsEnvMonitSamples(connect(store)(PolymerElement)))){static get properties(){return{tableDefinition:{type:Object,value:windowSettings},samplePendingRevisionTheTestingGroupMB:{type:Array,notify:!0},selectedObject:Object,callBackRefreshWindow:Object,selectedLanguage:{type:String},samplesStagesReduxVariables:{type:String,value:samplesStagesReduxVariables}}}stateChanged(state){this.selectedLanguage=state.app.user.appLanguage;if(null!=state.procDeploy){this.samplePendingRevisionTheTestingGroupMB=state.procDeploy.samplePendingRevisionTheTestingGroupMB}}static get template(){return html`            
        ...
        `}refreshWindow(){this.loadSamplingTable()}loadSamplingTable(){this.callBackRefreshWindow=this.refreshWindow.bind(this);this.getAllSamplesStageSampling(this.tableDefinition,this.samplesStagesReduxVariables.SAMPLES_PENDINGREVISION_TESTINGGROUPMB)}ready(){super.ready();this.loadSamplingTable()}}customElements.define("proc-deploy-sample-reviewsampletestinggroupmb",procDeploySampleReviewsampletestinggroupmb);// import {LitElement, html} from 'lit-element';
