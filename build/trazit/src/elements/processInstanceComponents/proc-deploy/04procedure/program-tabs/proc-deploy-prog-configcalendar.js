...
import "../../../../../../node_modules/@mpachnis/mp-calendar/mp-calendar.js";
import "../../../../internalComponents/Grids/vaadingrid-lit-singleselect.js";
import "../../03config/Programs/proc-deploy-progtab-calendar-settings.js";
import { programProgConfigCalendar_progConfigCalendarTableHeaderFields } from "../../03config/Programs/proc-deploy-progtab-calendar-settings.js";
class procDeployProgConfigcalendar extends FieldsMethods(
  connect(store)(PolymerElement)
) {
  stateChanged(state) {
    this.selectedLanguage = state.app.user.appLanguage;
    if (null != state.procDeploy.selectedProgram) {
  ...
  static get properties() {
    return {
      ...
      startDayNumber: { type: String }, // events: {type: Object, value:
      //     [{"title":"E01","content":"Muestreo ...","date":"2020-02-20","category":"blue", "color": "#000"},]},
      progConfigCalendarTableHeaderFields: {
        type: Object,
        value: programProgConfigCalendar_progConfigCalendarTableHeaderFields,
      },
      weekDaysDisabled: { type: Object, value: ["Sunday", "Saturday"] },
      tableTitle: {
        type: Object,
        value: {
          label_en: "Scheduled program locations",
          label_es: "Tabla de ubicaciones programadas para el programa",
        },
      },
      selectedProgram: { type: Object, observer: "setConfigScheduledCalendar" },
      displayCalendar: { type: Boolean, value: !1 },
      displayTable: { type: Boolean, value: !1 },
    };
  }
  static get template() {
    return html`
      ...
      <template is="dom-if" if="[[displayCalendar]]">
        <mp-calendar
          ...
          first-day-of-week="[[startDayNumber]]"
          day-labels="[[daysLabels]]"
          month-labels="[[monthsLabels]]"
          disable-prev-days="true"
          disable-next-days="true"
          theme="light-blue"
          disabled-days="[[weekDaysDisabled]]"
          events-object="[[selectedProgram.config_scheduled_calendar]]"
        ></mp-calendar>
      </template>
      ...
      <template is="dom-if" if="[[displayTable]]">
        ...
        <div>
          <vaadingrid-lit-singleselect
            class="grid"
            id="programCalendarGrid"
            headerfields="{{progConfigCalendarTableHeaderFields}}"
            rowcontainer="{{selectedProgram.config_scheduled_calendar}}"
