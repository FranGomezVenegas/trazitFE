...
import "../03config/Programs/proc-deploy-programmainview-settings.js";
import {
  FrontendEndpointsEnvMonitForPrograms,
  FrontendEndpointsEnvMonitForProductionLot,
} from "../01moduleFunctionality/endpoints-frontend-env-monit.js";
import {
  em_programs_tabs,
  programMain_programSelectionForm,
  programHome_defaultTab,
  tabsDefinition,
} from "../03config/Programs/proc-deploy-programmainview-settings.js";
import { setSelectedProgram } from "../02Redux/proc-deploy_actions.js"; //import './program-tabs/proc-deploy-prog-home.js';
...
import "./program-tabs/proc-deploy-prog-points-map.js"; // import './program-tabs/proc-deploy-prog-corrective-actions.js';
class procDeployPrograms extends FieldsMethods(
  TabsMethods(
    FrontendEndpointsEnvMonitForProductionLot(
      FrontendEndpointsEnvMonitForPrograms(connect(store)(PolymerElement))
    )
  )
) {
  stateChanged(state) {
  ...
  programSelected(e) {
    if ("programsList" == e.detail.name) {
      ...
      this.getSelectedProgramCorrectiveAction({
        programName: this.programs[e.detail.index].name,
      });
      var elem = this.shadowRoot.getElementById(
        "proc-deploy-corrective-actions"
  ...
  static get template() {
    return html`
      ...
      <div class="wrapper">
        <vaadin-button on-click="refresh"
      ...
      <div id="program_definition" class="programTabs">
        ...
        <iron-pages selected="[[currentSubTab]]" attr-for-selected="id">
          ...
          <proc-deploy-prog-configcalendar
            display-calendar="{{tabsDefinition.configCalendar.displayCalendar}}"
            display-table="{{tabsDefinition.configCalendar.displayTable}}"
            id="proc-deploy-config-calendar"
            selected-program="{{selectedProgram}}"
          >
          </proc-deploy-prog-configcalendar>
          <proc-deploy-prog-limits
            id="proc-deploy-limits"
            selected-program="{{selectedProgram}}"
          >
          </proc-deploy-prog-limits>
          <proc-deploy-prog-points-map
            id="proc-deploy-sampling-points-map"
            selected-program="{{selectedProgram}}"
          >
          </proc-deploy-prog-points-map>
          <proc-deploy-prog-points
            id="proc-deploy-sampling-points"
            selected-program="{{selectedProgram}}"
          >
          </proc-deploy-prog-points>
          <proc-deploy-prog-corrective-actions
            id="proc-deploy-corrective-actions"
            selected-program="{{selectedProgram}}"
          >
          </proc-deploy-prog-corrective-actions>
        </iron-pages>
