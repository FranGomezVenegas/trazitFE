...
import "../../../internalComponents/Grids/vaadin-grid-expand.js";
import {
...
import "./em-demo-a-batches.js";
import "../03config/em-demo-a-sample-incub-batch-settings.js";
import {
  sampleIncubBatchScreenSettings,
  sampleIncubBatchPendingIncub1,
  sampleIncubBatchPendingIncub2,
} from "../03config/em-demo-a-sample-incub-batch-settings.js"; //'../03config/config-process.js';
class emDemoASampleIncubBatch extends FieldsMethods(
  ...
  static get properties() {
    return {
      sampleIncubBatchScreenSettings: {
        type: Object,
        value: sampleIncubBatchScreenSettings,
      },
      incub1TableDefinition: {
        type: Object,
        value: sampleIncubBatchPendingIncub1,
      },
      incub2TableDefinition: {
        type: Object,
        value: sampleIncubBatchPendingIncub2,
      },
      samplesStagesReduxVariables: {
        type: String,
        value: samplesStagesReduxVariables,
      },
      allSamplesStageIncubation1: { type: Array, notify: !0 },
      allSamplesStageIncubation2: { type: Array, notify: !0 },
  ...
  static get template() {
    return html`
      ...
      <em-demo-a-webcomponent-env-monit-samples
        id="myElementsSample"
      ></em-demo-a-webcomponent-env-monit-samples>
      <!-- <vaadin-grid-expand></vaadin-grid-expand> -->
...
-->
      <div class="mainIncubBatchDiv">
        <template
          is="dom-if"
          if="[[sampleIncubBatchScreenSettings.displaySamplesPendingIncub1Tbl]]"
        >
          <div name="incub1Area">
            <template
              is="dom-if"
              if="[[incub1TableDefinition.tableTitle.display]]"
            >
              <p class="tableTitle">
                {{labelValue(selectedLanguage,
                incub1TableDefinition.tableTitle.label)}}
              </p>
            </template>
            <div name="incub1TableDefinitionButtons" class="buttonGroup">
              <template
                is="dom-if"
                if="[[incub1TableDefinition.displayRefreshButton]]"
              >
              ...
              <template
                is="dom-if"
                if="[[incub1TableDefinition.displayButtons]]"
              >
                <template
                  is="dom-repeat"
                  items="{{incub1TableDefinition.buttons}}"
                  as="currentfield"
                >
                  <field-controller
            ...
            <vaadingrid-lit-singleselect
              id="emdemoa-incub1Table"
              headerfields="{{incub1TableDefinition.sampleFieldToDisplay}}"
              rowcontainer="{{allSamplesStageIncubation1}}"
        ...
        <template
          is="dom-if"
          if="[[sampleIncubBatchScreenSettings.displaySamplesPendingIncub2Tbl]]"
        >
          <div name="incub2Area">
            <template
              is="dom-if"
              if="[[incub2TableDefinition.tableTitle.display]]"
            >
              <p class="tableTitle">
                {{labelValue(selectedLanguage,
                incub2TableDefinition.tableTitle.label)}}
              </p>
            </template>
            <div name="incub2TableDefinitionButtons" class="buttonGroup">
              <template
                is="dom-if"
                if="[[incub2TableDefinition.displayRefreshButton]]"
              >
                <vaadin-button id="refreshButton" on-click="refreshWindow"
                  ><iron-icon icon="refresh"></iron-icon
                ></vaadin-button>
              ...
              <template
                is="dom-if"
                if="[[incub2TableDefinition.displayButtons]]"
              >
                <template
                  is="dom-repeat"
                  items="{{incub2TableDefinition.buttons}}"
                  as="currentfield"
                >
                  <field-controller
                    id="{{currentfield.name}}"
            ...
            <vaadingrid-lit-singleselect
              id="emdemoa-incub2Table"
              headerfields="{{incub2TableDefinition.sampleFieldToDisplay}}"
              rowcontainer="{{allSamplesStageIncubation2}}"
              selected-object="{{selectedObject}}"
            >
            </vaadingrid-lit-singleselect>
          </div>
      ...
      <template
        is="dom-if"
        if="[[sampleIncubBatchScreenSettings.displayIncubBatchesTbl]]"
      >
        <div>
  ...
  loadWindowTables() {
    ...
    this.getAllSamplesStageSampling(
      ...
      this.samplesStagesReduxVariables.PENDING_INCUBATION1
    );
    this.getAllSamplesStageSampling(
      this.incub2TableDefinition,
      this.samplesStagesReduxVariables.PENDING_INCUBATION2
    );
    var elem = this.shadowRoot.getElementById("batches");
    if (elem) {
      elem.refreshWindow();
    } //        this.getAllSamplesStageIncubation1(this.incub1TableDefinition);
    //        this.getAllSamplesStageIncubation2(this.incub2TableDefinition);
  }
  stateChanged(state) {
    this.selectedLanguage = state.app.user.appLanguage;
    if (null != state.emDemoA) {
      this.allSamplesStageIncubation1 =
        state.emDemoA.allSamplesStageIncubation1;
      this.allSamplesStageIncubation2 =
        state.emDemoA.allSamplesStageIncubation2;
    }
  }
  ready() {
    super.ready();
...
customElements.define("em-demo-a-sample-incub-batch", emDemoASampleIncubBatch); // import {PolymerElement, html} from '@polymer/polymer/polymer-element';
