...
import "./genoma-instancia1-dialogmodal-buttons.js"; //import '@polymer/paper-checkbox/paper-checkbox';
import "../../03config/Dialogs/genoma-instancia1-dialogmodal-settings.js";
import { ModuleFunctionsGenoma } from "../../01moduleFunctionality/0module-functions-genoma.js";
import { dialogEnterVariableValueInteger } from "../../03config/Dialogs/genoma-instancia1-dialogmodal-settings.js";
/**
 ...
 * @demo
 *
 */ class GenomaInstancia1EnterVariableValueDialog extends ModuleFunctionsGenoma(
  PolymerElement
) {
  ...
  openListElement() {
    this.isList = !1;
    this.isInteger = !1;
    if (this.selectedObject == void 0 || this.selectedObject.type == void 0) {
      return;
    }
    if ("CATEGORICAL" == this.selectedObject.type.toUpperCase()) {
      this.isList = !0;
      return;
    }
    if ("INTEGER" == this.selectedObject.type.toUpperCase()) {
      this.isInteger = !0; //this.$.numSamples.value=this.selectedObject.value ;
      return;
    }
    return;
  }
      ...
      <div class="modal-content bgimg">
        ...
        <div id="newvalue">
          <template is="dom-if" if="{{isList}}">
            <template
              is="dom-repeat"
              items="{{convertAllowedValuesFldToArray(selectedObject.allowed_values)}}"
              as="currentListValue"
            >
              <paper-checkbox
                new-value="{{currentListValue}}"
                cur-object-type="{{currentstudycardelements.name}}"
                cur-object="{{curStudyFamily}}"
                on-change="selCheckObject"
                >{{currentListValue}}</paper-checkbox
              >
  ...
  selCheckObject(e) {
    if (!1 == e.currentTarget.ariaChecked) {
      this.newValue = "";
    }
    if ("true" == e.currentTarget.ariaChecked) {
      this.newValue = e.currentTarget.newValue + "";
    } else {
      this.newValue = "";
    }
    console.log(
  ...
  getNewValue() {
    ...
    if ("TEXT" == this.selectedObject.type.toUpperCase()) {
  ...
  dialogCanceled() {
    this.newValue = ""; //console.log('dialogCanceled', this.value);
    this.value = "confirmed";
    this.dispatchEvent(
      new CustomEvent("dialog-button-clicked", {
        bubbles: !0,
...
customElements.define(
  ...
  GenomaInstancia1EnterVariableValueDialog
);
