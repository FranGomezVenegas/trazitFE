...
import{store}from"../../../../store.js";import{ApiSettings}from"../../../../platform-mixins/apis/api-settings.js";import{tokenMixin}from"../../../../platform-mixins/store/token-mixin.js";import{ToastMethods}from"../../../../platform-mixins/functions/toast-methods.js";import{schema_name}from"../03config/config-process.js";import{GenomaModuleDefinition}from"./0module-backendfunctionality-available.js";import{activeProjects as activeProjects_genoma_instancia1,setSelectedStudy as setSelectedStudy_genoma_instancia1}from"../02Redux/genoma-instancia1_actions.js";export const FrontendEndpointsModuleGenoma=superClass=>class extends GenomaModuleDefinition(ApiSettings(ToastMethods(tokenMixin(superClass)))){getProjects(){this.callBackRefreshWindow=this.refreshWindow.bind(this);var data=[];this.callEndpoint(data,"ALL_ACTIVE_PROJECTS");//this.getActiveProductionLotsList();
}refreshSelectedStudyOnGetProjects(projects){var state=store.getState();if(state.genomaInstancia1.selectedStudy.name!=void 0){var selStudyName=state.genomaInstancia1.selectedStudy.name,selProjectName=state.genomaInstancia1.selectedStudy.project,selProject=projects.project.find(function(tab){return tab.name.toUpperCase()==selProjectName.toUpperCase()});if(selProject==void 0){return}var selStudy=selProject.study.find(function(tab){return tab.name.toUpperCase()==selStudyName.toUpperCase()});store.dispatch(setSelectedStudy_genoma_instancia1(selStudy));console.log("setSelectedStudy fired by getProjects")}}callEndpoint(data,endpointName){var endpoints_returningError=this.endpoints_returningError(),moduleEndpoints=[];//console.log('callEndpoint', 'data', data, 'endpointName', endpointName);
moduleEndpoints=this.endPoints();var endpointDefinition=moduleEndpoints.find(function(tab){return tab.actionName.toUpperCase()==endpointName.toUpperCase()});if(!endpointDefinition){this.toastErrorMessage(this.actionNameNotRecognized(endpointName.toUpperCase(),"FrontendEndpointsModuleGenoma"));return}if(endpointDefinition.apiCallDiscarded!=void 0&&!0==endpointDefinition.apiCallDiscarded){store.dispatch(activeProjects(data,endpointName));return}var apiUrl=endpointDefinition.apiurl;//backendUrl+frontEndGenomaStudyUrl; 
...
store.dispatch(activeProjects_genoma_instancia1(response.data));this.refreshSelectedStudyOnGetProjects(response.data);//store.dispatch(activeProjects_genoma_instancia1(response.data, endpointName));
