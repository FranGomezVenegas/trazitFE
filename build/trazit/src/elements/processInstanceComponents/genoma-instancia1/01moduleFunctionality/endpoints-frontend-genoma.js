...
import { GenomaModuleDefinition } from "./0module-backendfunctionality-available.js";
import {
  activeVariablesAndVariablesSet as activeVariablesAndVariablesSet_genoma_instancia1,
  activeProjects as activeProjects_genoma_instancia1,
  setSelectedStudy as setSelectedStudy_genoma_instancia1,
} from "../02Redux/genoma-instancia1_actions.js";
export const FrontendEndpointsModuleGenoma = (superClass) =>
  class extends GenomaModuleDefinition(
    ApiSettings(ToastMethods(tokenMixin(superClass)))
  ) {
    getProjects() {
      this.callBackRefreshWindow = this.refreshWindow.bind(this);
      var data = [];
      this.callEndpoint(data, "ALL_ACTIVE_PROJECTS"); //this.getActiveProductionLotsList();
    }
    ...
    refreshSelectedStudyOnGetProjects(projects) {
      ...
      if (state.genomaInstancia1.selectedStudy.name != void 0) {
        var selStudyName = state.genomaInstancia1.selectedStudy.name,
          selProjectName = state.genomaInstancia1.selectedStudy.project,
          selProject = projects.project.find(function (tab) {
            return tab.name.toUpperCase() == selProjectName.toUpperCase();
          });
        if (selProject == void 0) {
          return;
        }
    ...
    callEndpoint(data, endpointName) {
      var endpoints_returningError = this.endpoints_returningError(),
        moduleEndpoints = []; //console.log('callEndpoint', 'data', data, 'endpointName', endpointName);
      ...
      if (
        endpointDefinition.apiCallDiscarded != void 0 &&
        !0 == endpointDefinition.apiCallDiscarded
      ) {
      ...
      apiUrl =
        ...
        schema_name +
        "&actionName=" +
        endpointName; //console.log('process-us>api-sample>sampleAPIEndpointCall', data.schemaPrefix, data.actionName, apiUrl, data.paramsUrl);
      ...
      axios
        .get(apiUrl, {
          params: {
            //'schemaPrefix':schema_name, 'actionName':'ALL_ACTIVE_PROJECTS',
          ...
          if (200 == response.status) {
            ...
            switch (endpointName) {
              case "ALL_ACTIVE_VARIABLES_AND_VARIABLES_SET":
                store.dispatch(
                  activeVariablesAndVariablesSet_genoma_instancia1(
                    response.data
                  )
                );
                return;
              case "ALL_ACTIVE_PROJECTS":
                ...
                this.refreshSelectedStudyOnGetProjects(response.data); //store.dispatch(activeProjects_genoma_instancia1(response.data, endpointName));
