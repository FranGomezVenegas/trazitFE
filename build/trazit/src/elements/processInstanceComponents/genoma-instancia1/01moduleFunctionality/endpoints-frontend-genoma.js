...
var state=store.getState();if(state.genomaInstancia1.selectedStudy.name!=void 0){var selStudyName=state.genomaInstancia1.selectedStudy.name,selProjectName=state.genomaInstancia1.selectedStudy.project,selProject=projects.project.find(function(tab){return tab.name.toUpperCase()==selProjectName.toUpperCase()});if(selProject==void 0){return}var selStudy=selProject.study.find(function(tab){return tab.name.toUpperCase()==selStudyName.toUpperCase()});store.dispatch(setSelectedStudy_genoma_instancia1(selStudy));console.log("setSelectedStudy fired by getProjects")}}callEndpoint(data,endpointName){var endpoints_returningError=this.endpoints_returningError(),moduleEndpoints=[];//console.log('callEndpoint', 'data', data, 'endpointName', endpointName);
