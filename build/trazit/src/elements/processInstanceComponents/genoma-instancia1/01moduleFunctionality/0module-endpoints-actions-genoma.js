...
import { addNotification } from "../../../platformComponents/Redux/actions/notifications_actions.js";
import { diagnosticToNotification } from "../../../../platform-mixins/platform-functions/notification-obj.js";
...
import { schema_name } from "../03config/config-process.js"; //incubationMode
import { tokenMixin } from "../../../../platform-mixins/store/token-mixin.js";
import { userConfirmationMixin } from "../../../../platform-mixins/store/user-confirmation-mixin.js";
 ...
 * @mixinFunction
 * @polymer
 */ export const EndpointsActionsGenomaModule = (superClass) =>
  class extends userConfirmationMixin(
    GenomaModuleDefinition(ApiSettings(ToastMethods(tokenMixin(superClass))))
  ) {
    ...
    moduleActionTriggerNext(buttonDefinition, backEndData, moduleArea) {
      ...
      var buttonName = buttonDefinition.name,
        datas = [],
        areaActions = [];
      if (moduleArea.actionsFunction == void 0) {
      ...
      if (actionDefinition.dialogInfo.requiresDialog) {
        ...
        if (elem) {
          ...
          var message = [];
          message.message_en =
            "Not found any WebComponent called " +
            actionDefinition.dialogInfo.webComponentName;
          message.message_es =
            "No encontrado ning\xFAn WebComponent llamado " +
            actionDefinition.dialogInfo.webComponentName;
    ...
    moduleAPIEndpointCall(
      ...
      var endpoints_returningError = this.endpoints_returningError(),
        apiUrl =
          backendUrl + moduleArea.apiUrlForActions + "?" + data.paramsUrl;
      apiUrl =
        apiUrl +
        "&finalToken=" +
        this.getFinalToken() +
        "&schemaPrefix=" +
        schema_name; //console.log('process-us>api-sample>moduleAPIEndpointCall', data.schemaPrefix, data.actionName, apiUrl, data.paramsUrl);
      ...
      axios
        ...
        .then((response) => {
          console.log(
            "em-demo-a > api-env-monit.js > moduleAPIEndpointCall",
            "response.data",
            response.data
          );
