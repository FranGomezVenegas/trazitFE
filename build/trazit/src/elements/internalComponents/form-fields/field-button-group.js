...
import { store } from "../../../store.js"; // import {
//     addTab,
//     closeTab,
//     setCurrentTab
...
class FieldButtonGroup extends connect(store)(PolymerElement) {
  static get properties() {
    return {
      _page: String,
      currentProcedure: { type: Object, notify: !0 },
      currentProcedureEvent: { type: Object, notify: !0 },
  ...
  labelValue() {
    ...
    if ("en" == this.selectedLanguage)
      return this.procedure.label_en + "-" + this.field.label_en;
    if ("es" == this.selectedLanguage)
      return this.procedure.label_es + "-" + this.field.label_es;
  ...
  static get template() {
    return html`
      ...
      <paper-button-group
        id="{{buttonGroupName}}"
        on-click="optionPressed"
        selected="{{selectedActionName}}"
        attr-for-selected="item-name"
      >
        <template is="dom-repeat" items="[[field.items]]">
          <paper-button class="blue" item-name="[[item.keyName]]">
            {{_labelListValue(selectedLanguage, item.keyValue_en,
            item.keyValue_es)}}
          </paper-button>
        </template>
        <paper-button class="blue" item-name="SAMPLINGCOMMENTADD"
          >Add Comment</paper-button
        >
        <paper-button class="blue" item-name="SAMPLINGCOMMENTREMOVE"
          >Remove Comment</paper-button
        >
      </paper-button-group>
  ...
  clicked() {
    ...
    this.dispatchEvent(
      new CustomEvent("field-button-group-clicked", {
        bubbles: !0,
        composed: !0,
        detail: {
