    ...
    return html`
      ...
      <template is="dom-if" if="{{textTypePassword()}}">
        <vaadin-password-field
          ...
          value="{{value}}"
          >value="{{value}}"</vaadin-password-field
        >
      </template>
      <template is="dom-if" if="{{textTypeConfidential()}}">
        <textarea
          ...
          readonly
          label="{{labelValue(selectedLanguage, field)}}"
          value="{{confidentialMaskValue(selectedLanguage)}}"
          required="{{field.required}}"
          auto-validate
          tip="You are not authorized to see this information"
        ></textarea>
      </template>
    `;
  }
