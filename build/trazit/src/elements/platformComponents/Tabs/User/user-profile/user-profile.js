...
import { AuthenticationApi } from "../../../../../platform-mixins/apis/api-authentication.js";
import {
  changeUserPasswordForm,
  changeUserEsignForm,
  saveOpenTabsForm,
} from "./user-profile-settings.js";
import { ApiPlatform } from "../../../../../platform-mixins/apis/api-platform.js";
 ...
 * @demo
 *
 */ class UserProfile extends TabsMethods(
  ApiPlatform(AuthenticationApi(connect(store)(PolymerElement)))
) {
  stateChanged(state) {
  ...
  static get properties() {
    return {
      ...
      changeUserPasswordForm: { type: Array, value: changeUserPasswordForm },
      changeUserEsignForm: { type: Array, value: changeUserEsignForm },
        ...
        <div id="central">
          ...
          <div class="card">
            <div id="changePw" style="display:flex;">
              <template
                is="dom-repeat"
                items="{{changeUserPasswordForm}}"
                as="currentfield"
              >
                <field-controller
                  on-keydown="keyPressedChangePassword"
                  on-field-button-clicked="changePassword"
            ...
            <div id="changeEsign" style="display:flex;">
              <template
                is="dom-repeat"
                items="{{changeUserEsignForm}}"
                as="currentfield"
              >
                <field-controller
                  on-keydown="keyPressedChangeEsign"
                  on-field-button-clicked="changeEsign"
                  on-field-list-value-changed="onListChange"
                  id="{{currentfield.name}}"
                  field="{{currentfield}}"
                ></field-controller>
            ...
            <div id="changeEsign" style="display:flex;">
              <template
                is="dom-repeat"
                items="{{saveOpenTabsForm}}"
                as="currentfield"
              >
                <field-controller
                  on-keydown="keyPressed"
                  on-field-button-clicked="saveDefaultTabsOnLogin"
                  on-field-list-value-changed="onListChange"
                  id="{{currentfield.name}}"
                  field="{{currentfield}}"
                ></field-controller>
  ...
  changePassword(e) {
    ...
    this.$.platformelements.appActionTrigger(
      changeUserPasswordForm[1].name,
      selectedRow,
      changeUserPasswordForm[1]
    );
  }
  changeEsign(e) {
    var selectedRow = [];
    ...
    this.$.platformelements.appActionTrigger(
      changeUserEsignForm[1].name,
      selectedRow,
      changeUserEsignForm[1]
    );
  }
}
