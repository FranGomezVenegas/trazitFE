...
import {
  proceduresListPaneTitle,
  tabNotOpenableByCertification,
} from "./procedures-list-pane-settings.js";
import "./procedures-list-pane-style.js";
import { isTabOpenableWhenNotSopCertified } from "../../../../../config/platform/logic/platform-config.js";
import { addTab, setCurrentTab } from "../../../Redux/actions/tabs_actions.js";
...
class ProceduresListPane extends ToastMethods(
  ...
  static get properties() {
    return {
      ...
      tabNotOpenableByCertification: {
        type: Object,
        value: tabNotOpenableByCertification,
      },
      titleValue: String,
      titleIcon: String,
  ...
  static get template() {
    return html`
      ...
      >
        <template
          is="dom-repeat"
          items="{{procedureList.procedures}}"
          as="currprocedure"
        >
          <div class="title">
            <template
              is="dom-repeat"
              items="{{currprocedure.icons_up}}"
              as="currentfield"
            >
              <field-controller
                style="padding-top: 0px; padding-bottom: 0px;"
                id="{{currentfield.name}}"
                on-field-button-clicked="crearTab"
                tab-index="{{index}}"
                field="{{currentfield}}"
          ...
          <template
            is="dom-repeat"
            items="{{currprocedure.definition}}"
            as="currentfield"
          >
            <field-controller
              title="{{fieldTitlePendingSOP}}"
              style="padding-top: 0px; padding-bottom: 0px;"
              id="{{currentfield.name}}"
              on-field-tree-list-clicked="crearTab"
              tab-index="{{index}}"
              field="{{currentfield}}"
              procedure="{{currprocedure}}"
            ></field-controller>
          ...
          <div class="title">
            <template
              is="dom-repeat"
              items="{{currprocedure.icons_down}}"
              as="currentfield"
            >
              <field-controller
                style="padding-top: 0px; padding-bottom: 0px;"
