...
import { notificationsPaneTitle } from "../Components/Notifications/notifications-pane-settings.js";
import { isTabOpenableWhenNotSopCertified } from "../../../../config/platform/logic/platform-config.js"; //import {addTab,setCurrentTab} from '../../Redux/actions/tabs_actions';
//import {FieldsMethods} from '../../../../platform-mixins/functions/fields-methods';
//import {ToastMethods} from '../../../../platform-mixins/functions/toast-methods';
import { platformHeadertwoStyle } from "./platform-headertwo-settings.js";
import { platformHeaderFields } from "./platform-headertwo-settings.js";
 ...
 * @demo
 *
 */ class PlatformHeadertwo extends FieldsMethods(
  TabsMethods(connect(store)(LitElement))
) {
  stateChanged(state) {
    if (state.app.user) {
   ...
   */ constructor() {
    ...
    this.procedureList = [];
    this.appLogOut_logOutMessage = appLogOut_logOutMessage; //console.log('platformHeaderFields', this.platformHeaderFields);
  }
  static get styles() {
    return [
      // super.styles,
        ...
        <div class="left">
          <div class="logo">
            <img
              ...
              src="./images/header/trazit-removebg.png"
            />
          </div>
          <div class="logo">
            <!-- <img class="formFieldLogoCircle" src="./images/header/trazit-removebg.png"> -->
            ${this.platformHeaderFields.fieldsCenter.map(
              (item) => html`<p class="sessioninfo">
                <b>
                  ${this.labelValue(this.selectedLanguage, item)}
                  <!-- <field-controller .field="${item}" @field-button-clicked="${this
                    .doLogout}" 
          @avatar-item-clicked="${this
                    .platformHeaderRightClicked}"></field-controller> -->
                </b>
              </p>`
            )}
          </div>
        ...
        <div class="center">
                              ...
                              ${item.icons_up.map(
                                (iconup) => html`<li>
                                  <a
                                    .procedure="${item}"
                                    .procevent="${iconup}"
                                    @click="${this.procedureTab}"
                                  >
                                    <!--(item, iconup)-->
                                    <!-- <field-controller style="padding-top: 0px; padding-bottom: 0px;" id="${iconup.name}" @field-button-clicked="${this
                                      .crearTab}" tab-index="${item.index}" .field="${iconup}" .procedure="${item}"></field-controller> -->
                                    <!-- ${this.labelValue(
                                      this.selectedLanguage,
                                      iconup
                                    )}                        -->
                                    <paper-icon-button
                                      style="${this.iconStyleDefinition}"
                                      icon="${iconup.icon_name}"
                                      title="${this.labelValue(
                                        this.selectedLanguage,
                                        iconup
                                      )}"
                                      disabled="${iconup.read_only}"
                                      value="${iconup.name}"
                                    ></paper-icon-button>
                                  </a>
                                </li>`
                              ...
                              ${item.definition.map(
                                (item2) => html`<li>
                                  ...
                                  >
                                    ...
                                    <!--                <field-controller title="${this
                                      .fieldTitlePendingSOP}" style="padding-top: 0px; padding-bottom: 0px;" id="${item2.name}" @field-tree-list-clicked="${this
                                      .crearTab}" tab-index="${item.index}" .field="${item2}" procedure="${item}"></field-controller>            -->
                                  </a>
                                </li>`
                              )}
                            `
                              ...
                              ${item.icons_down.map(
                                (item2) => html`<li>
                                  ...
                                  >
                                    ...
                                    <paper-icon-button
                                      style="${this.iconStyleDefinition}"
                                      icon="${icons_down.icon_name}"
                                      title="${this.labelValue(
                                        this.selectedLanguage,
                                        icons_down
                                      )}"
                                      disabled="${icons_down.read_only}"
                                      value="${icons_down.name}"
                                    ></paper-icon-button>
                                  </a>
                                </li>`
                              )}
          ...
          `
                        ...
                        ${this.notificationPaneIsEmpty
                          ...
                          : html`
                              ${this.notifications.map(
                                (item) => html`<li>
                                  <a class="${this.textColor(item[1])}">
                                    ${this.notifText(item[1])}
                                  </a>
                                </li>`
                              )}
                            `}
              ...
              ${this.platformHeaderFields.sopOption.display
                ? html`
                    <li class="circle">
                      <a @click="${this.AllUserSOPClicked}"
                        >${this.tabLabelValue(
                          this.selectedLanguage,
                      ...
                      ${0 < this.contPendingUserSOPs
                        ? html`
                            <a
                              style="color:red;"
                              title="${this.tabLabelValue(
                                this.selectedLanguage,
                                SOP_pendingSOPTab
                              )}"
                              @click="${this.pendingSOPClicked}"
                              >${this.contPendingUserSOPs}</a
                            >
                          `
                        : ``}
                      <a
                        ...
                        @click="${this.AllUserSOPClicked}"
                        >${this.contAllUserSOPs}</a
                      >
                      ...
                      <ul>
                        ${this.platformHeaderFields.personalOption.options.map(
                          (item) => html`<li>
                            <a
                              .field="${item}"
                              @click="${this.platformHeaderRightClicked}"
                            >
                              ${"avatar" == item.type
                                ? html`
                                    <img
...
<!--                    <field-controller .field="${item}" @field-button-clicked="${
  ...
  crearTab(e) {
    ...
    var curTab = [];
    curTab.tabName =
      e.detail.procedure.name + "-" + e.detail.procEvent.lp_frontend_page_name;
