...
let loaderScript=document.querySelector("script[src=\"https://www.gstatic.com/charts/loader.js\"]");if(!loaderScript){// If the loader is not present, add it.
loaderScript=document.createElement("script");// Specify URL directly to pass JS compiler conformance checks.
 ...
 */export async function load(settings={}){await loaderPromise;const{version="current",packages=["corechart"],language=document.documentElement.lang||"en",mapsApiKey}=settings;return google.charts.load(version,{packages:packages,language:language,mapsApiKey:mapsApiKey})}/**
 ...
 */export async function dataTable(data){// Ensure that `google.visualization` namespace is added to the document.
await load();if(null==data){return new google.visualization.DataTable}else if(data.getNumberOfRows){// Data is already a DataTable
return data}else if(data.cols){// data.rows may also be specified
...
throw new Error("Data was empty.")}throw new Error("Data format was not recognized.")}/**
 * Creates new `ChartWrapper`.
 * @param container Element in which the chart will be drawn
 */export async function createChartWrapper(container){// Ensure that `google.visualization` namespace is added to the document.
await load();// Typings suggest that `chartType` is required in `ChartSpecs`, but it works
// without it.
return new google.visualization.ChartWrapper({container:container})}//# sourceMappingURL=loader.js.map