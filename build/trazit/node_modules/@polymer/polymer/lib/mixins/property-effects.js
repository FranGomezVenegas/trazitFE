...
// the effect metadata
if(0>cmp){start=mid+1}else if(0<cmp){end=mid-1}else{idx=mid;break}}if(0>idx){idx=end+1}queue.splice(idx,0,info)},enqueueEffectsFor=(prop,computeEffects,queue,order,hasPaths)=>{const rootProperty=hasPaths?root(prop):prop,fxs=computeEffects[rootProperty];if(fxs){for(let i=0;i<fxs.length;i++){const fx=fxs[i];if(fx.info.lastRun!==dedupeId&&(!hasPaths||pathMatchesTrigger(prop,fx.trigger))){fx.info.lastRun=dedupeId;insertEffect(fx.info,queue,order)}}}};/**
