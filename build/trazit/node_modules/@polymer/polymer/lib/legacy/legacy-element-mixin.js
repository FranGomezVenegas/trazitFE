 ...
 */export const LegacyElementMixin=dedupingMixin(base=>{// TODO(kschaaf): Note, the `@implements {Polymer_DirMixin}` is required here
...
if(!this.hasOwnProperty(JSCompiler_renameProperty("__observedAttributes",this))){this.__observedAttributes=[];register(this.prototype)}return this.__observedAttributes}else{return observedAttributesGetter.call(this).concat(DISABLED_ATTR)}}// NOTE: Inlined for perf from version of DisableUpgradeMixin.
...
if(this.__isUpgradeDisabled&&null==value){this._initializeProperties();this.__isUpgradeDisabled=!1;if(wrap(this).isConnected){this.connectedCallback()}}}else{super.attributeChangedCallback(name,old,value,namespace);this.attributeChanged(name,old,value)}}}/**
...
if(legacyOptimizations&&this.hasAttribute(DISABLED_ATTR)){this.__isUpgradeDisabled=!0}else{let proto=Object.getPrototypeOf(this);if(!proto.hasOwnProperty(JSCompiler_renameProperty("__hasRegisterFinished",proto))){this._registered();// backstop in case the `_registered` implementation does not set this
