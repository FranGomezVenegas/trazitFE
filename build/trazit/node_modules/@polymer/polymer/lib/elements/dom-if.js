...
let observer=new MutationObserver(()=>{if(wrap(this).querySelector("template")){observer.disconnect();this.__render()}else{throw new Error("dom-if requires a <template> child")}});observer.observe(this,{childList:!0});return!1}this.__template=template}return!0}/**
...
let lastChild=wrap(this).previousSibling;if(lastChild!==children[children.length-1]){for(let i=0,n;i<children.length&&(n=children[i]);i++){wrap(parentNode).insertBefore(n,this)}}}}return!0}/**
 ...
 */export const DomIf=fastDomIf?DomIfFast:DomIfLegacy;customElements.define(DomIf.is,DomIf);